...
class AptekaOtSkladaSpider(scrapy.Spider):
    ...
    custom_settings = {
        "LOG_LEVEL": "INFO",
        "ITEM_PIPELINES": {"apteka_ot_sklada.pipelines.AptekaOtSkladaPipeline": 300},
    }
    ...
    def start_requests(self):
        yield scrapy.Request(
            "https://apteka-ot-sklada.ru/api/region", callback=self.parse_region_data
        )
    ...
    def parse_region_data(self, response):
        ...
        for region in regions_list:
            if "Томск" in region.get("name"):
                id = region.get("id")
                yield scrapy.Request(
                    f"https://apteka-ot-sklada.ru/api/region/{id}", callback=self.parse
                )
    ...
    def parse_categories(self, response, **kwargs):
        for category_url in self.start_urls:
            yield scrapy.Request(category_url, callback=self.parse_categories_pages)
    ...
    def parse_categories_pages(self, response):
        product_urls = response.xpath(
            "//div[@class='goods-grid__inner']//a[@itemprop='url']/@href"
        ).getall()
        for product_url in product_urls:
            yield response.follow(product_url, callback=self.parse_product_page)
        pagination = response.xpath(
            "//li[@class='ui-pagination__item ui-pagination__item_next']/a/@href"
        ).get()
        if pagination:
            yield response.follow(pagination, callback=self.parse_categories_pages)
    ...
    def parse_product_page(self, response):
        ...
        breadcrumbs = response.xpath(
            "//ul[@class='ui-breadcrumbs__list']//li/a/span/span/text()"
        ).getall()[2:]
        price = self.get_price(response)
        availability = self.get_availability(response, json_data)
        assets = self.get_assets(response)
        brand = response.xpath(
            "//div[@itemprop='manufacturer']/span[@itemtype='legalName']/text()"
        ).get()
    ...
    def get_price(self, response):
        ...
        if len(price_xpath) == 1:
            price = response.xpath(
                "//div[@class='goods-offer-panel__price']/span//text()"
            ).get()
            price = float(self.clean_string(price).replace(" ", ""))
            return {"current": price, "original": price, "sale_tag": ""}
        elif len(price_xpath) == 2:
            current_price = response.xpath(
                "//div[@class='goods-offer-panel__price']/span[1]//text()"
            ).get()
            original_price = response.xpath(
                "//div[@class='goods-offer-panel__price']/span[2]//text()"
            ).get()
            ...
            return {"current": 0, "original": 0, "sale_tag": ""}

    def get_json_data(self, response):
        string_json = response.xpath(
    ...
    def get_assets(self, response):
        images = response.xpath(
            "//ul[@class='goods-gallery__preview-list']/li//img/@src"
        ).getall()
    ...
    def get_marketing_tags(self, response):
        tags = response.xpath(
            "//ul[@class='goods-tags__list goods-tags__list_direction_horizontal']//li/span/text()"
        ).getall()
    ...
    def get_metadata(self, response, json_data):
        ...
        description = [
            self.clean_string(x) for x in description if self.clean_string(x) != ""
        ]
        description = " ".join(description)
        country_of_origin = response.xpath(
            "//div[@itemprop='manufacturer']/span[@itemtype='location']/text()"
        ).get()
